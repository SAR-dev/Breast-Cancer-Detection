...
function crop_multiple_images(images_folder)
%% get a valid folder name
if ~exist('images_folder','var')  
       images_folder=('C:\VT004\screenshots\post_bmode_figure');
...
image_files_5=image_files(5); % the first file
image_file_5_full_name=fullfile(images_folder,image_files_5.name);
% then load this file using imread
I5=imread(image_file_5_full_name);
imshow(I5),set(gcf,'Name',image_files_5.name)
drawnow
...
[~,rect]=imcrop(I5); % and keep the rctangle position
close(gcf)
...
folder_name_to_save2=('C:\VT004\screenshots\region_of_interest_crop')
...
for i=1:N
    ...
    msg=sprintf('%d out of %d files processed', i, N);
